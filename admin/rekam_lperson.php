...
$idp = $_GET['nrm'];
$daftar = myquery("SELECT * FROM daftar WHERE id_pasien='$idp' ORDER BY tanggal DESC");
...
$pdf->Cell(4, 0.8, 'No. RM : '. $idp, 1, 0, 'C');
$pdf->Cell(11, 0.8, '', 0, 0, 'C');
$pdf->Cell(4, 0.8, date_format(date_create($daf['tanggal']), 'j M Y'), 1, 1, 'C');
$pdf->ln(0.35);
...
$pdf->Cell(2.5, 0.7, ($anamnesa) ? ': '. $ana['darah'] .' mmHg' : '', 'T', 0, 'L');
$pdf->Cell(1, 0.7, ' TN', 'T', 0, 'L');
$pdf->Cell(2.5, 0.7, ($anamnesa) ? ': '. $ana['nadi'] .' x/m' : '', 'R,T', 1, 'L');

$lahir = date_format(date_create($daf['tgl_lahir']), 'Y');
$umur = date('Y') - $lahir . " Tahun";
$pdf->Cell(3, 0.7, 'Tanggal Lahir', 'B,L,T', 0, 'L');
$pdf->Cell(9, 0.7, ': '. $daf['tgl_lahir'] .' -- '. umur($daf['tgl_lahir'], " Tahun ", " Bulan ", " Hari ") , 'B,R,T', 0, 'L');
$pdf->Cell(1, 0.7, ' Suhu', '', 0, 'L');
$pdf->Cell(2.5, 0.7, ($anamnesa) ? ': '. $ana['suhu'] . utf8_decode('°C') : '', '', 0, 'L');
$pdf->Cell(1, 0.7, ' SPO', '', 0, 'L');
$pdf->Cell(2.5, 0.7, ($anamnesa) ? ': '. $ana['spo'] .' %' : '', 'R', 1, 'L');

$pdf->Cell(3, 0.7, 'Status', 'B,L,T', 0, 'L');
$pdf->Cell(9, 0.7, ': '. $pas['status'], 'B,R,T', 0, 'L');
$pdf->Cell(1, 0.7, ' BB', 'B', 0, 'L');
$pdf->Cell(2.5, 0.7, ($anamnesa) ? ': '. $ana['berat'] .' kg' : '', 'B', 0, 'L');
$pdf->Cell(1, 0.7, ' TB', 'B', 0, 'L');
$pdf->Cell(2.5, 0.7, ($anamnesa) ? ': '. $ana['tinggi'] .' cm' : '', 'B,R', 1, 'L');
...
$pdf->Cell(2.25, 0.7, ($periksa) ? ': ' . $per['ma_anemis'] : '', 'B,R,T', 1, 'L');
...
$pdf->Cell(2.25, 0.7, ($periksa) ? ': ' . $per['ma_ikterik'] : '', 'B,R,T', 1, 'L');
...
$pdf->Cell(2.25, 0.7, ($periksa) ? ': ' . $per['le_kelenjar'] : '', 'B,R,T', 1, 'L');

$pdf->Cell(3, 0.7, 'Alamat', 'B,L,T', 0, 'L');
$pdf->Cell(9, 0.7, ': '. $daf['alamat'] .", ". $daf['kel'] .", ". $daf['kec'] .", ". $daf['kota'], 'B,R,T', 0, 'L');
$pdf->Cell(1.75, 0.7, '', 'B,T', 0, 'L');
$pdf->Cell(3, 0.7, 'JVP', 'B,T', 0, 'L');
$pdf->Cell(2.25, 0.7, ($periksa) ? ': ' . $per['le_jvp'] : '', 'B,R,T', 1, 'L');
...
$pdf->SetFont('Arial','',8);
$pdf->cell(1, 0.7, $no, 1, 0, 'C');
$pdf->cell(2, 0.7, $b['kode'], 1, 0, 'L');
$pdf->cell(5, 0.7, substr($b['nama'], 0, 27), 1, 0, 'L');
$pdf->cell(3.5, 0.7, substr($b['jenis'], 0, 17), 1, 0, 'L');
...
$pdf->cell(4.75, 0.7, 'Farmasis', 1, 1, 'C');
$pdf->SetFont('Arial','',8);
$pdf->cell(4.75, 1, ($daftar) ? $daf['user'] : '', 1, 0, 'C');
$pdf->cell(4.75, 1, ($daftar) ? $daf['status'] : '', 1, 0, 'C');
$pdf->cell(4.75, 1, ($periksa) ? $per['user'] : '', 1, 0, 'C');
$apoteker = myquery("SELECT DISTINCT apoteker FROM tindak WHERE id_daftar='$idd'");
// $apo = $apoteker[0];
// var_dump($apoteker[0]);
$pdf->cell(4.75, 1, ($apoteker) ? $apoteker[0]['apoteker'] : '', 1, 1, 'C');
